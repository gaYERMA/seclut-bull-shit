function getChromeTabs(tabOptions) {
    return new Promise((resolve, reject) => {
        chrome.tabs.query(tabOptions, function (tabs) {
            chrome.runtime.lastError && console.log("Cannot get chrome tabs: " + chrome.runtime.lastError.message), resolve(tabs)
        })
    })
}
...
async function getWindow(windowId) {
    return new Promise((resolve, reject) => {
        chrome.windows.get(windowId, window => {
            chrome.runtime.lastError ? reject(chrome.runtime.lastError.message) : resolve(window)
        })
    })
}
...
function createWindow(urls) {
    return new Promise(resolve => {
        ...
        urls && (data.url = urls), chrome.windows.create(data, window => {
            resolve(window)
        })
    })
}
...
async function getTab(tabId) {
    return new Promise((resolve, reject) => {
        chrome.tabs.get(tabId, function (tab) {
            chrome.runtime.lastError && reject(chrome.runtime.lastError.message), resolve(tab)
        })
    })
}
...
function closeTab(tabId) {
    return new Promise((resolve, reject) => {
        chrome.tabs.remove(tabId, function () {
            chrome.runtime.lastError && console.log("Error closing tab: " + chrome.runtime.lastError.message), resolve()
        })
    })
}
...
function captureVisibleTab() {
    return new Promise((resolve, reject) => {
        chrome.tabs.captureVisibleTab(function (screenshotUrl) {
            chrome.runtime.lastError && reject(chrome.runtime.lastError.message), void 0 === screenshotUrl && reject("No screenshot captured"), resolve(screenshotUrl)
        })
    })
}
async function getConfig() {
    return new Promise(resolve => {
        chrome.storage.local.get(null, function (items) {
            chrome.runtime.lastError && (console.log("Error loading config from storage: " + chrome.runtime.lastError.message), resolve(new Config({}))), resolve(new Config(items))
        })
    })
}
async function getStoredUrls() {
    return new Promise(resolve => {
        chrome.storage.local.get(STORAGE_URLS, urls => {
            chrome.runtime.lastError ? (console.log("Failed to load urls from local storage"), resolve()) : resolve(urls[STORAGE_URLS])
        })
    })
}
async function getEmail() {
    return "function" == typeof getSubstitutedEmail ? Promise.resolve(getSubstitutedEmail()) : IS_EXTENSION && chrome.identity ? new Promise(resolve => {
        const prepare = function (email) {
            return email && email.trim().length > 0 ? email : null
        };
        chrome.declarativeNetRequest ? chrome.identity.getProfileUserInfo({
            accountStatus: "ANY"
        }, function (userInfo) {
...
async function getDirectoryDeviceId() {
    return new Promise((resolve, reject) => {
        chrome.enterprise && chrome.enterprise.deviceAttributes ? chrome.enterprise.deviceAttributes.getDirectoryDeviceId(function (deviceId) {
            console.log("Start with requested directory device ID=" + deviceId), resolve(deviceId)
        }) : resolve(null)
    })
}
...
function clearAllAlarms() {
    return new Promise(resolve => {
        chrome.alarms.clearAll(wasCleared => {
            wasCleared ? console.log("All alarms cleared") : (console.log("Alarms not cleared"), chrome.runtime.lastError && console.log("Alarms clear error: " + chrome.runtime.lastError.message)), resolve()
        })
    })
}
...
function injectScript(tabId) {
    return new Promise((resolve, reject) => {
        chrome.tabs.executeScript(tabId, {
            ...
            chrome.runtime.lastError ? (console.log("Failed to inject scripts"), reject(chrome.runtime.lastError.message)) : resolve()
        })
    })
}

function updateTab(tabId, properties) {
    return "number" != typeof tabId ? Promise.resolve() : new Promise((resolve, reject) => {
...
function openLockWindow() {
    return new Promise((resolve, reject) => {
        chrome.windows.create({
            ...
            chrome.runtime.lastError ? reject(chrome.runtime.lastError.message) : resolve(window.id)
        })
    })
}
...
async function getLocation() {
    return new Promise(resolve => {
        navigator.geolocation.getCurrentPosition(position => {
            ...
            console.log("Lat: " + coordinates.latitude.toFixed(4) + " Lon: " + coordinates.longitude.toFixed(4)), resolve(location)
        }, error => {
...
class Config {
    ...
    async clearWhitelist() {
        return this.lockedToCourseWorkResources = !1, this.store(STORAGE_WHITELIST, null)
    }
    async saveBlockedInfo(blockedMessage, modeType, notificationInterval, allowCaptureScreen, allowContactAdmin) {
        let savedItems = {};
        blockedMessage ? (savedItems[STORAGE_SCREEN_BLOCK_MESSAGE] = blockedMessage, this.items[STORAGE_SCREEN_BLOCK_MESSAGE] = blockedMessage) : (savedItems[STORAGE_SCREEN_BLOCK_MESSAGE] = this._defaultBlockMessage, this.items[STORAGE_SCREEN_BLOCK_MESSAGE] = this._defaultBlockMessage), modeType && (savedItems[STORAGE_BLOCKED_MODE_TYPE] = modeType, this.items[STORAGE_BLOCKED_MODE_TYPE] = modeType);
        let diagString = "Stored screen block modeType: " + modeType + ", message: " + blockedMessage;
        return void 0 !== allowCaptureScreen && (savedItems[STORAGE_IS_SCREEN_CAPTURE_ALLOWED] = allowCaptureScreen, this.items[STORAGE_IS_SCREEN_CAPTURE_ALLOWED] = allowCaptureScreen, diagString += ", capture screen: " + allowCaptureScreen), void 0 !== allowContactAdmin && (savedItems[STORAGE_IS_CONTACT_ADMIN_ALLOWED] = allowContactAdmin, this.items[STORAGE_IS_CONTACT_ADMIN_ALLOWED] = allowContactAdmin, diagString += ", allow contact admin: " + allowContactAdmin), diagString += ", notification interval: ", void 0 !== allowCaptureScreen && notificationInterval !== LOC_PARAM_DONT_CHANGE && (savedItems[STORAGE_NOTIFICATION_INTERVAL] = notificationInterval, this.items[STORAGE_NOTIFICATION_INTERVAL] = notificationInterval, diagString += notificationInterval), console.log(diagString), this.bulkStore(savedItems)
    }
    ...
    store(key, value) {
        return value ? (this.items[key] = value, new Promise(resolve => {
            chrome.storage.local.set({
                ...
                chrome.runtime.lastError ? console.log("Failed to save object " + JSON.stringify(value) + " for key '" + key + "'. Error is: " + chrome.runtime.lastError.message) : console.log("Stored " + JSON.stringify(value) + " for key: " + key), resolve()
            })
        })) : (delete this.items[key], new Promise(resolve => {
            chrome.storage.local.remove([key], function () {
                chrome.runtime.lastError ? console.log("Failed to remove key '" + key + "'. Error is: " + chrome.runtime.lastError.message) : console.log("Key '" + key + "' removed"), resolve()
            })
        }))
    }
    bulkStore(object) {
        return new Promise(resolve => {
            chrome.storage.local.set(object, function () {
                chrome.runtime.lastError ? console.log("Failed to bulk save '" + JSON.stringify(object) + "'. Error is: " + chrome.runtime.lastError.message) : console.log("Items " + JSON.stringify(object) + " saved"), resolve()
            })
        })
    }
    bulkDelete(keys) {
        return new Promise(resolve => {
            chrome.storage.local.remove(keys, function () {
                chrome.runtime.lastError ? console.log("Failed to remove keys " + keys + " Error is: " + chrome.runtime.lastError.message) : console.log("Keys " + keys + " removed"), resolve()
            })
        })
    }
}
const st = async function () {
    if (chrome.alarms.clearAll(), "function" == typeof setTitle && setTitle(), IS_EXTENSION ? (iconManager = new IconManager, iconManager.noActiveClassIcon(), config = await getConfig()) : config = new Config({}), router = new Router(config.serverUrl), config.userEmail = await getEmail(), IS_KIOSK && (requestBlockScreen(config.blockedText, config.blockedModeType, config.notificationInterval, config.isScreenCaptureAllowed, config.isContactAdminAllowed), updateVersion()), IS_EXTENSION) {
        const storedUrls = await getStoredUrls();
        console.log("INIT URL STORAGE WITH URLS=" + JSON.stringify(storedUrls)), urlStorage = new UrlStorage(storedUrls)
    }
...
async function obtainDirectoryDeviceId() {
    ...
    directoryDeviceId ? (config.directoryDeviceId = directoryDeviceId, await config.saveIsChromeBook(!0)) : "function" == typeof substituteDeviceId && substituteDeviceId()
}
const stp = function () {
...
class Router {
    ...
    async sendCommand(message) {
        ...
        try {
            return console.log("Sending command=" + JSON.stringify(message)), await this.send(message, FIRE_PATH), !0
        } catch (error) {
            return console.log("Failed to send command: " + JSON.stringify(message) + ".  Error=" + error), !1
        }
    }
    async readCommands(messageAck) {
        return Object.assign(messageAck, {
            version: VERSION,
            udid: config.deviceUdid,
            command: "ack"
        }), console.log("Sending ack=" + JSON.stringify(messageAck)), await this.send(messageAck, POLLING_PATH)
    }
    async send(message, path) {
        return new Promise((resolve, reject) => {
            const xhr = new XMLHttpRequest;
            xhr.timeout = HTTP_REQUEST_TIMEOUT, xhr.upload.addEventListener("error", function (event) {
                reject("XHR: Error in request upload")
            }), xhr.ontimeout = function () {
                reject("XHR: Request timeout")
            }, xhr.open("POST", this.url + path), xhr.setRequestHeader(HEADER_REG_CODE, message.udid), xhr.setRequestHeader("Content-Type", "application/json; charset=UTF-8"), xhr.onreadystatechange = async function () {
                if (this.readyState === XMLHttpRequest.DONE)
                    if (0 === this.status) reject("Status 0");
                    else if (200 === this.status)
                        if (this.responseText && this.responseText.length > 0) try {
                    ...
                    else this.status === HTTP_PRECONDITION_FAILED ? resolve({
...
async function processCommand(command) {
    ...
    return void 0 !== action ? actionResult = await action(command).catch(error => {
        if (shouldRethrow(error)) throw error;
        console.log("Failed to process action for '" + command.requestType + "': " + error)
    }) : console.log("Got unknown message: " + command.requestType), actionResult || Router.getAck(command)
}
async function pollServer() {
    console.log("Long polling started"), chrome.alarms.create(ALARM_SEND_URLS, {
        periodInMinutes: 1
    });
    let errorCounter = 0,
        message = null;
    for (; ;) {
        try {
            message || (message = Router.getAck()), message = await processResponse(await router.readCommands(message)), errorCounter = 0
        } catch (error) {
            ...
            if ((errorCounter += 1) >= 3) {
                chrome.alarms.create(ALARM_RESTART_POLLING, {
                    delayInMinutes: 1
        ...
        if (!config.isLoggedIn) {
            chrome.alarms.create(ALARM_REPEAT_LOGIN, {
                delayInMinutes: 1
...
function alarmProcessing(alarm) {
    switch (console.log("Fired alarm " + alarm.name), alarm.name) {
        case ALARM_SHOW_WARNING:
            config.isScreenLocked || requestBlockScreen(config.blockedText, config.blockedModeType, LOC_PARAM_DONT_CHANGE, config.isScreenCaptureAllowed, config.isContactAdminAllowed);
            break;
        ...
        case ALARM_WINDOWS_AGENT_CONNECT:
            chrome.alarms.clear(ALARM_WINDOWS_AGENT_CONNECT), nativeAgent.connect(nativeAgentMessageListener), nativeAgent.isConnected && nativeAgent.handshake();
            break;
        default:
...
function shouldRethrow(error) {
    ...
    for (const sample of rethrowableErrors)
        if (hasText(error, sample)) return !0;
    return !1
}
...
function sendPing() {
    ...
    xhr.open("POST", serverUrl), xhr.setRequestHeader(HEADER_REG_CODE, config.deviceUdid), xhr.setRequestHeader("Content-Type", "application/json; charset=UTF-8");
    const message = {
        udid: config.deviceUdid,
        logString: msg,
        logNum: lg.logCount,
...
async function sendLoggedUrls() {
    ...
    if (activeSession) {
        ...
        if (tabs.length > 0) {
            const tab = tabs[0];
            void 0 !== tab.id && tab.id !== chrome.tabs.TAB_ID_NONE && (tabId = tab.id);
            const url = tab.url;
            void 0 !== url && (domain = parseDomain(url))
        }
        tabId > 0 && domain && urlStorage.saveUrl(activeSession, new Url(domain, tabs[0].title))
    }
    const message = {
        udid: config.deviceUdid,
    ...
    try {
        await sendUrlHttpRequest(message), console.log("Urls sent successfully"), urlStorage.purge(), sendLoggedUrls.isSending = !1
    } catch (error) {
        sendLoggedUrls.isSending = !1, console.log("Error while sending urls: " + error)
    }
}
async function sendUrlHttpRequest(message) {
    return new Promise((resolve, reject) => {
        ...
        xhr.timeout = HTTP_REQUEST_TIMEOUT, xhr.upload.addEventListener("error", function (event) {
            ...
            this.readyState === XMLHttpRequest.DONE && (200 === this.status ? resolve() : (0 === this.status || this.status >= 400) && reject("Failed to send urls to the server: " + this.status))
        };
        const serverUrl = config.serverUrl + "/agent/chrome/url";
        xhr.open("POST", serverUrl), xhr.setRequestHeader(HEADER_REG_CODE, config.deviceUdid), xhr.setRequestHeader("Content-Type", "application/json; charset=UTF-8"), xhr.send(JSON.stringify(message))
    })
}
async function loginToServer(showErrorPage) {
    console.log("Initiate login at " + new Date), await clearAllAlarms();
    try {
        if (await processResponse(await getOrgUrlIfNeeded()), await processResponse(await sendAuthenticateIfNeeded()), !await sendLogin()) return console.log("Failed to login. Repeat in 1 minute"), void chrome.alarms.create(ALARM_REPEAT_LOGIN, {
            periodInMinutes: 1
        });
        IS_EXTENSION && (chrome.browserAction.onClicked.removeListener(loginToServer), chrome.runtime.onMessage.removeListener(pageMessageListener)), "undefined" != typeof lg && (lg.logCount = 0), config.pingCount = 0, config.loginErrorReason = null, pollServer()
    } catch (error) {
        hasText(error, ERROR_DEVICE_NOT_FOUND) ? (console.log("Failed to authenticate: " + error), console.log("Logout and login again to connect"), showErrorPage && config.loginErrorReason && config.loginErrorReason !== REASON_WINDOWS_NOT_ALLOWED && await showNotFoundScreen()) : hasText(error, ERROR_CANNOT_UPDATE_EXTENSION) ? (console.log(error), processExtensionUpgradeError()) : (console.log("Failed to login: " + error), chrome.alarms.create(ALARM_REPEAT_LOGIN, {
            periodInMinutes: 1
        }))
    }
}
...
function getOrgUrlIfNeeded() {
    ...
    if (console.log("Request server url=" + requestServerUrl + " config.serverUrl=" + config.serverUrl + " udid=" + deviceId), !requestServerUrl) return Promise.resolve();
    const message = {
    ...
    return console.log("Get org url message=" + JSON.stringify(message)), router.send(message, POLLING_PATH)
}
async function sendAuthenticateIfNeeded() {
    ...
    return config.directoryDeviceId ? message.udid = config.directoryDeviceId : message.byod = !0, console.log("Send AUTHENTICATE=" + JSON.stringify(message)), router.send(message, POLLING_PATH)
}
...
async function sendScreenshot(dataUri, message) {
    ...
    void 0 !== message.tabUrl && (serverUrl += "&tabUrl=" + encodeURIComponent(message.tabUrl)), void 0 !== message.tabTitle && (serverUrl += "&tabTitle=" + encodeURIComponent(message.tabTitle)), performSendScreenshotRequest(serverUrl, blob)
}
async function performSendScreenshotRequest(serverUrl, blob) {
    return new Promise(resolve => {
        ...
        }, xhr.open("POST", serverUrl), xhr.setRequestHeader(HEADER_REG_CODE, config.deviceUdid), xhr.setRequestHeader("Content-Type", "image/jpg"), xhr.onreadystatechange = function () {
            this.readyState === XMLHttpRequest.DONE && (this.status >= 200 && this.status < 400 ? console.log("Screenshot send complete to url=" + serverUrl) : console.log("Status=" + this.status + ". Failed to upload screenshot to url=" + serverUrl), resolve(204 !== this.status))
        }, xhr.send(blob)
    })
}
...
function buildServerUrlForScreenshot(urlPath) {
    ...
    return -1 !== serverUrl.indexOf("uid=") ? serverUrl += "&udid=" + config.deviceUdid : serverUrl += "?udid=" + config.deviceUdid, config.userEmail && (serverUrl += "&userEmail=" + encodeURIComponent(config.userEmail)), serverUrl
}
...
function resizeImage(dataUri, maxWidth, maxHeight) {
    return new Promise((resolve, reject) => {
        if (0 === maxWidth && 0 === maxHeight) console.log("Image will not be resized"), resolve(dataUri);
        else {
            ...
            img.onload = function (event) {
                ...
                let scaleFactor;
                1 === (scaleFactor = 0 !== widthFactor && 0 !== heightFactor ? widthFactor < heightFactor ? widthFactor : heightFactor : 0 !== widthFactor ? widthFactor : heightFactor) && (console.log("Image will not be resized"), resolve(dataUri)), resolve(steppedResize(img, scaleFactor))
            }, img.src = dataUri
        }
...
function steppedResize(img, scaleFactor) {
    ...
    for (let i = 1; i <= nOfsteps; i++) {
        ...
        1 === i ? (tCanvas.width = scaledWidth, tCanvas.height = scaledHeight, tCtx.drawImage(img, 0, 0, scaledWidth, scaledHeight)) : tCtx.drawImage(tCanvas, 0, 0, cur.width, cur.height, 0, 0, scaledWidth, scaledHeight), cur = {
            width: scaledWidth,
            height: scaledHeight
        }
    }
    ...
    return ctx.imageSmoothingEnabled = !0, ctx.imageSmoothingQuality = "high", ctx.drawImage(tCanvas, 0, 0, cur.width, cur.height, 0, 0, canvas.width, canvas.height), canvas.toDataURL("image/jpeg")
}
...
class Url {
    constructor(domain, tabTitle, isBlocked) {
        this.domain = domain, this.tabTitle = tabTitle, this.isBlocked = !!isBlocked, this.timestamp = Date.now()
    }
}
class UrlStorage {
    constructor(urls) {
        this.urls = "object" == typeof urls ? urls : {}, this.lastVisitedUrl = null
    }
    saveUrl(sessionId, url) {
        sessionId || (sessionId = "blocked");
        let sessionUrls = this.urls[sessionId];
        if (sessionUrls || (sessionUrls = [], this.urls[sessionId] = sessionUrls), !this.isLast(url))
            if (this.lastVisitedUrl = url, url.domain.length < 1e3 && (!url.tabTitle || url.tabTitle.length < 255)) sessionUrls.push(url);
            else {
                const shortUrl = new Url(url.domain.substr(0, 1e3), url.tabTitle ? url.tabTitle.substr(0, 255) : null, url.isBlocked);
                sessionUrls.push(shortUrl)
            } chrome.storage.local.set({
                [STORAGE_URLS]: this.urls
            }, () => {
                chrome.runtime.lastError && console.log("Failed to save urls to local storage. Error is: " + chrome.runtime.lastError.message)
            })
    }
    isLast(url) {
    ...
    purge() {
        this.urls = {}, this.lastVisitedUrl = null, chrome.storage.local.remove(STORAGE_URLS, () => {
            chrome.runtime.lastError ? console.log("Failed to clear urls from local storage: " + chrome.runtime.lastError.message) : getStoredUrls().then(urls => {
                void 0 !== urls ? console.log("Failed to clear urls from local storage") : console.log("Urls cleared from local storage")
            })
        })
    }
}
chrome.runtime.onUpdateAvailable.addListener(function (details) {
    IS_EXTENSION && (console.log("Extension update available, will reload. Current version=" + VERSION), chrome.runtime.reload())
});
class IconManager {
    constructor() {
    ...
    clearRaiseHandIcon() {
        this.sharesScreen ? this.shareScreenIcon() : this.active ? this.activeClassIcon() : this.noActiveClassIcon()
    }
...
async function updateLockStatus(clearTabsCloseWarning) {
    ...
    for (const tab of tabs)
        if (!isSiteLocked || void 0 === tab.url || !isDomainExcluded(tab.url)) try {
            await sendLockStatusToTab(tab, "onApply", clearTabsCloseWarning), await updateTab(tab.id, {
                muted: isScreenLocked
            })
        } catch (error) {
...
function updateWindowListeners() {
    if (config.isClassSessionActive) {
        const needFocusListener = (config.isScreenLocked || config.maximizeFocusedWindow) && !chrome.windows.onFocusChanged.hasListener(windowFocusChangeListener),
            needBoundsListener = config.maximizeFocusedWindow && !chrome.windows.onBoundsChanged.hasListener(windowBoundsChangeListener),
            needRemoveListener = config.isScreenLocked && !chrome.windows.onRemoved.hasListener(windowRemoveListener);
        needFocusListener && chrome.windows.onFocusChanged.addListener(windowFocusChangeListener), needBoundsListener && (chrome.windows.onBoundsChanged.addListener(windowBoundsChangeListener), chrome.windows.onCreated.addListener(windowBoundsChangeListener)), needRemoveListener && chrome.windows.onRemoved.addListener(windowRemoveListener)
    } else chrome.windows.onFocusChanged.removeListener(windowFocusChangeListener), chrome.windows.onRemoved.removeListener(windowRemoveListener), chrome.windows.onBoundsChanged.removeListener(windowBoundsChangeListener), chrome.windows.onCreated.removeListener(windowBoundsChangeListener)
}
async function windowFocusChangeListener(param) {
    if (!config.maximizeFocusedWindow || config.isScreenLocked || "number" != typeof param || param === chrome.windows.WINDOW_ID_NONE || param === config.blockWindowId) {
        if (config.isScreenLocked && config.isChromeBook) {
            ...
            void 0 !== param && ("number" != typeof param || param !== chrome.windows.WINDOW_ID_NONE && param === config.blockWindowId) || await updateWindowFullscreen(config.blockWindowId)
        }
    } else await updateWindow(param, {
        state: "maximized"
    })
}
async function windowRemoveListener(param) {
    config.isScreenLocked && config.blockWindowId === param && setTimeout(async () => {
        try {
            config.blockWindowId = await openLockWindow(), await updateWindowFullscreen(config.blockWindowId)
        } catch (error) {
...
async function windowBoundsChangeListener(window) {
    if ("normal" === window.type && window.id !== config.blockWindowId) try {
        await updateWindow(window.id, {
            state: "maximized"
        })
    } catch (error) {
...
function nativeAgentMessageListener(message) {
    ...
    switch (action) {
        case AGENT_CLOSE_APP:
            ...
            message.isClosed || (response.isClosed = !1, response.error = message.error), router.sendCommand(response);
            break;
        case AGENT_HANDSHAKE:
            "undefined" != typeof lg && nativeAgent.updateLogging(lg.logEnabled);
            const version = message.version;
            console.log("Native agent communication established. Version=" + version), router.sendCommand({
...
async function checkMaxTabs(tab) {
    if (config.maxOpenTabs > 0) {
        return (await getChromeTabs({})).length > config.maxOpenTabs ? (void 0 !== tab.id ? await closeTab(tab.id) : console.log("Cannot close tab, no tab.id"), Promise.resolve(!1)) : Promise.resolve(!0)
    }
...
function tabListenerOnActivate(activeTabInfo) {
    ...
    if (null != config.activeTabId) {
        if (config.activeTabId !== activeTabId && config.isChatActive) {
            ...
            if (sendCommandToTab(config.activeTabId, messageToClose).then(() => {
                console.log("Command to close chat in tabId=" + config.activeTabId + " sent. New active tabId=" + activeTabId), config.activeTabId = activeTabId
            }).catch(error => {
...
function tabListenerOnCreate(tab) {
    null !== config.conferenceId && "loading" === tab.status || tab.url && isOwnPageUrl(tab.url) || checkMaxTabs(tab).then(shallContinue => shallContinue ? sendChatStatusToTab(tab, "onCreate") : Promise.reject("Tabs limit exceeded")).catch(error => {
        console.log("Catched on tab create listener error: " + JSON.stringify(error))
    })
}

function tabListenerOnUpdate(tabId, changeInfo, tab) {
    if ("complete" === changeInfo.status) {
        if (null !== config.conferenceId && (void 0 === tab.url || isOwnPageUrl(tab.url))) return;
        checkMaxTabs(tab).then(() => sendChatStatusToTab(tab, "onUpdate")).then(() => {
...
async function sendLockStatusToTab(tab, messageType, clearTabsCloseWarning) {
    ...
    if ("number" == typeof tab.id) {
        if (!isAllowed(tab.url, ["https?:\\/\\/chrome\\.google\\.com\\/webstore\\/?.*?"], !1) && (config.isScreenLocked || config.isSiteLocked || null !== config.announce)) return await closeTab(tab.id), Promise.resolve();
        const showWarning = null !== config.closeTabsTimeout && !clearTabsCloseWarning || null,
            tabLockMessage = {
                command: "lockStatus",
...
async function siteLock(urls, shouldCloseTabs, notOpenTabs = !1) {
    try {
        let openedTabs;
        if (await updateLockStatus(), shouldCloseTabs && (openedTabs = await getChromeTabs({})), !notOpenTabs) {
            const windows = await getWindows();
            windows && 0 !== windows.length ? await openSites(urls) : await createWindow(urls)
        }
...
function updateIconAndTitle(isSessionActive, hasWindowsAgent) {
    iconManager || (iconManager = new IconManager), isSessionActive ? (iconManager.activeClassIcon(), setTitle(TITLE_CLASS_SESSION)) : (iconManager.noActiveClassIcon(), setTitle())
}
...
function processExtensionUpgradeError() {
    chrome.alarms.create(ALARM_RESTART_APP, {
        delayInMinutes: 5
...
function clearTeacherMessage() {
    ...
    config.announce = null, config.teacherId = null, updateTabListeners(), updateLockStatus().then(router.sendCommand(message))
}
...
function updateMessageListener(isSessionActive) {
    isSessionActive ? (chrome.runtime.onMessage.addListener(pageMessageListener), console.log("MESSAGE LISTENER ADDED")) : (chrome.runtime.onMessage.removeListener(pageMessageListener), console.log("MESSAGE LISTENER REMOVED"))
}

function pageMessageListener(request, sender, response) {
    switch (console.log("Got message from tab=" + JSON.stringify(sender.tab) + " Message=" + JSON.stringify(request)), request.action) {
        case "popup":
        ...
        case "clearTabsCloseWarning":
            updateTabListeners(), updateLockStatus(!0), console.log("Tabs close warning cleared by student"), response();
            break;
...
function processShareScreenResponse(request) {
    ...
    if (router.sendCommand(message), isConfirmed) {
        const deviceId = request.deviceId,
            shareScreenUrl = config.serverUrl + "/agent/sharescreen/upload?did=" + deviceId;
        config.shareScreenHandler = setInterval(shareScreen, 4e3, shareScreenUrl, 1024), iconManager.shareScreenIcon()
    }
}
async function shareScreen(shareScreenUrl, maxWH) {
    let isRequestedFromAgent = !1;
    if (nativeAgent.isConnected && (isRequestedFromAgent = nativeAgent.requestScreenshot(shareScreenUrl, maxWH, maxWH)), isRequestedFromAgent) console.log("Screenshot requested from native agent");
    else {
        try {
        ...
        performSendScreenshotRequest(shareScreenUrl, await resizeImage(config.capturedScreen, maxWH, maxWH)) || stopSharingScreen()
    }
}
...
function processPopupMessages(request, sender, response) {
    ...
    switch (request.popup) {
        case "hello":
            return getChromeTabs({
                ...
                if (tabs && tabs.length > 0) {
                    ...
                    url && !url.toLowerCase().includes("newtab") && (canStart = config.canStartChat)
                }
            }).catch(error => {
        ...
        case "handRaiseClick":
            config.isHandRaised = !config.isHandRaised, response({
                raiseHandEnabled: config.canRaiseHand,
                isHandRaised: config.isHandRaised
            }), config.isHandRaised ? iconManager.raiseHandIcon() : iconManager.clearRaiseHandIcon();
            break;
        case "startChatClick":
            response(), getChromeTabs({
                currentWindow: !0,
                active: !0
            }).then(tabs => {
                if (tabs && tabs.length > 0) {
                    ...
                    chrome.tabs.sendMessage(tabs[0].id, chatMessage), console.log("Sent message to tabId=" + tabs[0].id)
...
function processNotFoundPageMessages(request, sender, response) {
    response({
        reason: config.loginErrorReason
    })
}
...
function processChatMessages(request, sender, response) {
    switch (request.command) {
        case "getChatMessages":
            config.hasUnreadChatMessages = !1, response({
        ...
        case "sendMessage":
            ...
            config.chatMessages.push(studentMessage), router.sendCommand(chatMessage).catch(error => {
...
function processConferenceMessages(request, sender, response) {
    switch (request.command) {
        case "disconnect":
            console.log("Video conference is disconnected"), config.conferenceToken = null;
            const userDisconnectedMessage = {
                response: COMMAND_CONFERENCE_DISCONNECT
            };
            !0 === request.external && (userDisconnectedMessage.external = !0, config.clearWhitelist().then(() => {
                updateTabListeners(), updateLockStatus()
            })), config.conferenceTabId && closeTab(config.conferenceTabId).then(() => config.conferenceTabId = null), router.sendCommand(userDisconnectedMessage).catch(error => {
                console.log("Message=" + userDisconnectedMessage + " not delivered to the server. Error=" + error)
            }), response();
            break;
        default:
            response({
...
function requestCallback(details) {
    ...
    if (config.whitelist && config.lockedToCourseWorkResources && !details.type.includes("frame")) return {
        cancel: !1
    };
    ...
    if (-1 !== url.indexOf("/_/chrome/newtab")) return {
        cancel: !1
    };
    ...
    if (config.whitelist) !(shouldAllow = isAllowed(url, config.whitelist, !0)) && config.lockedToCourseWorkResources && isGoogleInitiated(details) && (shouldAllow = !0), redirectUrl += "?t=w";
    else if (config.blocklist || config.customBlocklist) {
        const shouldAllowOrg = !config.blocklist || isAllowed(url, config.blocklist, !1),
            shouldAllowCustom = !config.customBlocklist || isAllowed(url, config.customBlocklist, !1);
        !(shouldAllow = shouldAllowOrg && shouldAllowCustom) && details.type.includes("frame") && urlStorage.saveUrl(config.sessionId, new Url(url, null, !0)), shouldAllowOrg ? shouldAllowCustom || (redirectUrl += "?t=" + btoa(encodeURIComponent(config.customBlocklistName))) : redirectUrl += "?t=o"
    }
    return shouldAllow ? {
        cancel: !1
    } : {
...
function checkDomain(domain) {
    ...
    for (const pattern of allowed)
        if (domain.match(pattern)) return !0;
    return !1
}
...
function completedRequestCallback(details) {
    ...
    if ((!domain || -1 === domain.indexOf("chrome-extension")) && domain) {
        ...
        }, 5e3) : urlStorage.saveUrl(activeSession, new Url(domain, null))
    }
}
...
function isGoogleInitiated(details) {
    if ("string" == typeof details.initiator) {
        const initiator = details.initiator;
        return "https://classroom.google.com" === initiator || "https://docs.google.com" === initiator
    }
    return !1
}
const filter = {
    ...
    info = ["blocking"];
var urlStorage = null;
chrome.webRequest.onBeforeRequest.addListener(requestCallback, filter, info), console.log("Extension log here");
...
class NativeAgentCommunicator {
    constructor() {
        this.onDisconnectListener = this.onDisconnectListener.bind(this), this.onMessageListener = this.onMessageListener.bind(this)
    }
    connect(listener) {
        this.listener = listener, this.port = chrome.runtime.connectNative(NATIVE_HOST_NAME), this.port.onMessage.addListener(this.onMessageListener), this.port.onDisconnect.addListener(this.onDisconnectListener)
    }
    onMessageListener(message) {
        console.log("Native agent sent a message=" + JSON.stringify(message)), this.listener(message)
    }
    onDisconnectListener() {
        chrome.runtime.lastError && console.log("Native agent disconnected due to error=" + JSON.stringify(chrome.runtime.lastError)), this.port = null, this.listener = null
    }
    get isConnected() {
    ...
    sendMessage(message) {
        if (this.port) try {
            this.port.postMessage(message), console.log("Native agent received a message=" + JSON.stringify(message))
        } catch (error) {
            hasText(error.message, ERROR_DISCONNECTED_PORT) ? this.port = null : console.log("Native agent message sent error=" + error.message)
        }
        return null != this.port
    }
    ...
    requestScreenshot(url, maxWidth, maxHeight) {
        ...
        return maxWidth && (message.maxWidth = maxWidth), maxHeight && (message.maxHeight = maxHeight), this.sendMessage(message)
    }
...
async function actionCloseApp(message) {
    const appName = message.app,
        userId = message.userId;
    if (!nativeAgent.isConnected || !nativeAgent.closeApp(appName, userId)) return console.log("Cannot close '" + appName + "' app, native agent not connected"), {
        seq: message.seq,
        response: message.requestType,
        app: appName,
        isClosed: !1,
...
async function actionChatMessage(message) {
    config.chatMessages.push(message.chatMessage), config.hasUnreadChatMessages = !0, updateTabListeners();
    try {
...
async function actionClassSession(message) {
    ...
    if (config.maximizeFocusedWindow = maximizeFocused, isSessionActive && config.sessionId === sessionId) console.log("Same session. Continue");
    else if (isSessionActive || null !== config.sessionId) {
        if (isSessionActive || await sendLoggedUrls(), isSessionActive && isPremium ? (nativeAgent.connect(nativeAgentMessageListener), nativeAgent.isConnected && nativeAgent.handshake()) : nativeAgent.isConnected && nativeAgent.shutdown(), config.shareScreenHandler && stopSharingScreen(), closeSharedScreenTab(), await updateClassSessionState(sessionId, message, nativeAgent.isConnected), config.conferenceId = null, config.conferenceToken = null, showNotifications && (isSessionActive ? showNotification(NOTIFICATION_SESSION_ID, "Class session is active", "Your device could be monitored and remotely managed by a teacher") : showNotification(NOTIFICATION_SESSION_ID, "No active class session", "Your device is not monitored and cannot be remotely managed by a teacher")), isSessionActive && await logCurrentTabUrl(sessionId), maximizeFocused) {
...
async function updateClassSessionState(sessionId, message, hasWindowsAgent) {
    ...
    isSessionActive ? chrome.webRequest.onCompleted.hasListener(completedRequestCallback) || chrome.webRequest.onCompleted.addListener(completedRequestCallback, {
        urls: ["<all_urls>"],
        types: ["main_frame"]
    }) : chrome.webRequest.onCompleted.removeListener(completedRequestCallback), null != config.closeTabsTimeout && (clearTimeout(config.closeTabsTimeout), config.closeTabsTimeout = null), config.sessionId = sessionId;
    const allowHandRaise = !!message.allowHandRaise,
        ...
        maxOpenTabs = "number" == typeof parseInt(message.maxOpenTabs) ? parseInt(message.maxOpenTabs) : 0,
        chatMessages = message.chatMessages,
        forceOpenChat = !!message.forceOpenChat,
        canStartChat = !!message.canStartChat,
        playChatAlert = !!message.playChatAlertToStudent,
        isPremium = !!message.premium;
    config.canRaiseHand = allowHandRaise, config.isHandRaised = !1, config.announce = null, config.hasUnreadChatMessages = !1, config.chatMessages = chatMessages || [], config.forceOpenChat = forceOpenChat, config.canStartChat = canStartChat, config.playChatAlert = playChatAlert, config.maxOpenTabs = maxOpenTabs, updateIconAndTitle(isSessionActive, hasWindowsAgent), updatePopup(isPremium && isSessionActive), updateTabListeners(), updateMessageListener(isSessionActive), await updateChatStatus(), await updateLockStatus(), closeTabsOnStart && ("number" == typeof closeTabsWaitSeconds && closeTabsWaitSeconds > 0 ? (await warnAboutTabsClose(closeTabsWaitSeconds), config.closeTabsTimeout = setTimeout(async function () {
        console.log("Close tabs on session start after timeout"), await closeTabs(await getChromeTabs({}), !config.isChromeBook), config.closeTabsTimeout = null
    }, 1e3 * closeTabsWaitSeconds)) : (console.log("Close tabs on session start"), await closeTabs(await getChromeTabs({}), !config.isChromeBook)))
}

function stopSharingScreen() {
    clearInterval(config.shareScreenHandler), config.capturedScreen = null, config.shareScreenHandler = null, iconManager.noShareScreenIcon(), showNotification(NOTIFICATION_SCREEN_SHARING_ID, "Screen sharing ended", "Your teacher has stopped sharing your screen")
}
async function logCurrentTabUrl(activeSession) {
    const save = function (tabs, sessionId) {
        ...
        domain && -1 !== domain.indexOf("chrome-extension") || urlStorage.saveUrl(sessionId, new Url(domain, tab.title))
    },
        tabs = await getChromeTabs({
            lastFocusedWindow: !0,
            active: !0
...
async function actionConference(message) {
    ...
    conferenceId ? config.conferenceId === conferenceId && null != config.conferenceToken || (config.conferenceId = message.conferenceId, requestVideoConferenceToken(config.conferenceId).catch(error => console.log("Error while requesting video conference token " + error))) : (null !== config.conferenceId && (await config.clearWhitelist(), updateTabListeners(), await updateLockStatus()), config.conferenceId = null, config.conferenceToken = null)
}
async function actionConferenceToken(message) {
    ...
    if (null === config.conferenceToken) {
        ...
        const lockToSession = message.lockToSession,
            conferenceUrl = chrome.extension.getURL("conference/index.html");
        if (lockToSession) {
            const whitelist = ["^https://alfa-openvidu.techpilotlabs.com.*", "^https://streaming.deviceconsole.securly.com.*"];
            await config.saveBlockedInfo(null, PARAM_MODE_NONE), await config.saveWhitelist(whitelist), updateTabListeners(), await updateLockStatus()
        }
        try {
...
async function actionGetTabs(message) {
    ...
    if (nativeAgent.isConnected) {
        nativeAgent.requestAppList(message.userId) && (console.log("App list requested from native agent"), isAppListRequested = !0)
    }
    let activeTab = null;
    const activeTabs = await getChromeTabs({
        lastFocusedWindow: !0,
        active: !0
    });
    ...
    let tabsArray = [],
        activeTabIndex = -1;
    return tabs.forEach(function (tab, index, array) {
        const t = {
            id: tab.id,
            url: tab.url,
            title: tab.title
        };
        null != activeTab && tab.id === activeTab.id && (activeTabIndex = index), tabsArray.push(t)
    }), -1 !== activeTabIndex && tabsArray.splice(0, 0, tabsArray.splice(activeTabIndex, 1)[0]), {
        seq: message.seq,
        userEmail: config.userEmail,
        isHandRaised: config.isHandRaised,
...
async function actionLockScreen(message) {
    console.log("Lock screen requested and confirmed"), await config.clearWhitelist(), await config.saveBlockedInfo(message.screenBlockMessage, PARAM_MODE_BLOCK), updateTabListeners(), updateWindowListeners();
    try {
        config.blockWindowId = await openLockWindow(), await updateWindowFullscreen(config.blockWindowId)
    } catch (error) {
...
async function actionUnlock(message) {
    if (console.log("Unlock requested and confirmed"), await config.saveBlockedInfo(null, PARAM_MODE_NONE), await config.clearWhitelist(), updateTabListeners(), updateWindowListeners(), 0 !== config.blockWindowId) try {
...
async function actionTeacherMessage(message) {
    config.announce = message.teacherMessage, config.teacherName = message.teacherName, config.teacherId = message.teacherId, updateTabListeners(), await updateLockStatus();
...
async function actionBlocklist(message) {
    ...
    message.blocklist && (blocklist = message.blocklist), message.customBlocklist && (customBlocklist = message.customBlocklist, customBlocklistName = message.customBlocklistName), await config.saveBlocklist(blocklist), await config.saveCustomBlocklist(customBlocklist, customBlocklistName)
}
...
async function actionSiteLock(message) {
    console.log("Got site lock command"), await config.saveBlockedInfo(null, PARAM_MODE_NONE), await config.saveWhitelist(message.whitelistPatterns), config.lockedToCourseWorkResources = !!message.courseWorkResources, updateTabListeners(), await siteLock(message.urls, message.closeTabs, message.notOpenTabs)
}
async function actionUpgrade() {
    return new Promise(async (resolve, reject) => {
        if (console.log("Needs update check. Current version=" + VERSION), config.updateCheckDate.getTime() < (new Date).getTime()) {
            console.log("Checking for extension update");
            const nextCheckDate = new Date;
            nextCheckDate.setTime(nextCheckDate.getTime() + 36e5), await config.saveUpdateCheckDate(nextCheckDate), chrome.runtime.requestUpdateCheck(async status => {
                if (status === UPDATE_STATUS_AVAILABLE) return console.log("Extension update available, will reload"), void chrome.runtime.reload();
                if (status === UPDATE_STATUS_THROTTLED) {
                    const extendedDate = new Date;
                    extendedDate.setTime(extendedDate.getTime() + 144e5), await config.saveUpdateCheckDate(extendedDate)
                }
                reject(ERROR_CANNOT_UPDATE_EXTENSION + ". Status=" + status)
            })
        } else reject(ERROR_CANNOT_UPDATE_EXTENSION + ". Next check date is " + config.updateCheckDate)
    })
}
async function actionWindowsAgentUpgrade(message) {
    nativeAgent.requestUpgrade(message.version, message.url, message.sha) && chrome.alarms.create(ALARM_WINDOWS_AGENT_CONNECT, {
        delayInMinutes: 2
    })
}
...
async function actionAuthConfirm(message) {
    ...
    console.log("DeviceUdid set to: " + deviceUdid), IS_KIOSK || IconManager.clearBadge(), await config.saveDeviceUdid(deviceUdid)
}
async function actionDeviceNotFound(message) {
    throw "function" == typeof updateContactAdminButtonVisibility ? updateContactAdminButtonVisibility(!1) : IconManager.markNotAuthorized(), config.loginErrorReason = message.reason, await config.saveDeviceUdid(null), IS_EXTENSION && (chrome.runtime.onMessage.addListener(pageMessageListener), console.log("MESSAGE LISTENER ADDED"), chrome.browserAction.onClicked.addListener(loginFromButton)), await clearAllAlarms(), ERROR_DEVICE_NOT_FOUND
}
async function actionChangeName(message) {
    "function" == typeof updateDeviceInfo && updateDeviceInfo(config.deviceUdid, message.deviceName), await config.saveDeviceName(message.deviceName)
}
async function actionChangeServerUrl(message) {
    await config.saveServerUrl(message.serverUrl), router.updateUrl(message.serverUrl)
}
async function actionLogUpdate(message) {
    const logEnabled = message.shouldSendLogs;
    if ("undefined" != typeof lg && (lg.logEnabled = logEnabled), logEnabled ? chrome.alarms.create(ALARM_SEND_PING, {
        periodInMinutes: 5
    }) : chrome.alarms.clear(ALARM_SEND_PING), nativeAgent.isConnected) {
        nativeAgent.updateLogging(logEnabled) && console.log("Native agent logging updated")
    }
    console.log("Logging updated to " + logEnabled + " at " + new Date)
}
async function actionLoginConfirmation(message) {
    if (IS_EXTENSION && (config.studentId = message.studentId, config.studentName = message.studentName), IS_KIOSK) {
        ...
        await config.saveDeviceInfo(orgName, deviceName, lostModeType, screenBlockMessage, notificationInterval, allowContactAdmin, allowScreenCapture), updateOrgAndScreen(), updateContactAdminButtonVisibility(!0 === config.isContactAdminAllowed), updateDeviceInfo(config.deviceUdid, deviceName)
    }
    config.isLoggedIn = !0, console.log("Complete login routine. Screen " + (config.isScreenLocked ? "is locked with text '" + config.blockedText + "'" : " is not locked"))
}
